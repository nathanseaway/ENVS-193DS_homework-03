...
Figure 1. Distribution and means of frequency of sunscreen application by attendance to beach (yes or no). black points represent individual daily number of times sunscreen was applied. Yellow points represent mean daily sunscreen application by attendance type. Graph also displays standard deviation in mean sunscreen application. Data from "sunscreen_fixed.csv" by Nathan Seaway from personal experience.

d\.

\`\`\`{r}

sunscreen_table \<- sunscreen \|\> #renames suncreen to to sunscreen_table

group_by(Beach)\|\> #takes sunscreen application number and groups it by beach attendance

summarize(Mean_Sunscreen_Application =mean(Frequency_Sunscreen_Application), #creates column named mean sunscreen application which is the mean of the frequency of sunscreen application by beach attendance variable

Stand_Dev= sd(Frequency_Sunscreen_Application)) #creates column named stand dev which is the standard deviation of the mean of the frequency of sunscreen application by beach attendance variable
